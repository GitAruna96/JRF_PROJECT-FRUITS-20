...
def train_epoch(model, src_train_loader, loss_fn, optimizer, device):
    ...
    for X, y in src_train_loader:
        print(f"train_epoch - Type of X: {type(X)}, Type of y: {type(y)}")
        print(f"train_epoch - X shapes: {[t.shape for t in X] if isinstance(X, list) else X.shape}, y shape: {y.shape}")
        if isinstance(X, list):
            # Select the first tensor that matches the batch size of y
            for t in X:
                if t.shape[0] == y.shape[0]:
                    X = t
                    break
            else:
                raise ValueError(f"No tensor in X matches batch size of y ({y.shape[0]})")
            print(f"train_epoch - Selected X shape: {X.shape}")
...
def evaluation(model, trg_evaluate_loader, loss_fn, device): 
    ...
    with torch.no_grad():
        for X, y in trg_evaluate_loader: 
            #print(f"evaluation - Type of X: {type(X)}, Type of y: {type(y)}")
            #print(f"evaluation - X shapes: {[t.shape for t in X] if isinstance(X, list) else X.shape}, y shape: {y.shape}")
            if isinstance(X, list):
                # Select the first tensor that matches the batch size of y
                for t in X:
                    if t.shape[0] == y.shape[0]:
                        ...
                        raise ValueError(f"No tensor in X matches batch size of y ({y.shape[0]})")
                print(f"evaluation - Selected X shape: {X.shape}")
            X, y = X.to(device), y.to(device)
            y_pred = model(X)
...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_fruits_cls_reverse_results_{timestamp}.csv')
...
def main():
    ...
    src_train_loader = get_loader(name_dataset = 'fruitsP', batch_size = batch_size, train=True)
    trg_evaluate_loader = get_loader(name_dataset = 'fruitsO', batch_size = batch_size, train=False)

    for X, y in src_train_loader:
        print(f"Batch X type: {type(X)}, shape: {X.shape if isinstance(X, torch.Tensor) else [t.shape for t in X]}")
        print(f"Batch y type: {type(y)}, shape: {y.shape}")
        break
    ...
    for model_name in model_names:
        ...
        for epoch in range(no_epochs):
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                counter = 0
                torch.save(model.state_dict(), f'fruits_cls_{model_name}.pth')
