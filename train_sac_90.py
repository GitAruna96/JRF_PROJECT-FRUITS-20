...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_fruits_sac_90_results_{timestamp}.csv')
...
def main():
    ...
    for model_name in model_names: 
        ...
        model = create_model('swin_tiny_patch4_window7_224', pretrained = True, num_classes = output_classes)
        model.to(device)
        ...
        for epoch in range(no_epochs):
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                counter = 0
                torch.save(model.state_dict(), f'fruits_sac_90_{model_name}.pth')
