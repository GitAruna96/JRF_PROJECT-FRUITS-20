...
def train_epoch(model, src_loader, trg_loader, loss_fn, optimizer, device, cls_weight, ssc_weight, temperature, pseudo_weight, pseudo_batches=None):
    ...
    pseudo_iter = iter(pseudo_batches) if pseudo_batches else None
...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_ssc_pseudo_results.csv')
...
def main():
    ...
    for epoch in range(no_epochs):
        ...
        if test_acc > best_acc:
            ...
            torch.save(model.state_dict(), os.path.join(output_dir, 'best_swin_ssc_pseudo_model.pth'))
