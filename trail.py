...
def save_checkpoint(model, optimizer, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_sched_epoch_{epoch+1}_{test_acc:.2f}.pth')
   ...
   # Initialize model, optimizer, and loss functions
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            ...
            # Save regular model
            torch.save(model.state_dict(), os.path.join(output_dir, f'f_sac_rev_pred_best_{model_name}.pth'))
