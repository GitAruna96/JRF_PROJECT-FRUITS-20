...
import numpy as np
import pandas as pd
from ssc_data_load import get_loader
import torch
...
def train_epoch(model, src_loader, trg_loader, loss_fn, optimizer, device, ssc_weight, temperature, cls_weight):
    ...
    for X_src, y_src in src_loader:
        ...
        (X_trg_views, _) = next(trg_loader_iter)
...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_swin_ssc_results.csv')
...
def main():
    ...
    src_train_loader = get_loader(name_dataset='fruitsO', batch_size=batch_size, train=True)
    trg_train_loader = get_loader(name_dataset='fruitsP', batch_size=batch_size, train=True)
    trg_evaluate_loader = get_loader(name_dataset='fruitsP', batch_size=batch_size, train=False)

    # Class imbalance analysis - FIXED: torch.tensor (not tensors)
    ...
    all_source_labels = torch.tensor(all_source_labels)  # FIXED: tensor not tensors
    ...
    X_sample, y_sample = next(iter(src_train_loader))
    print(f"Input shape: {X_sample.shape}")
    print(f"Label shape: {y_sample.shape}")
    print(f"Label values: {y_sample.unique()}")
    if len(y_sample.shape) > 1 and y_sample.shape[1] == no_classes:
        print("Warning: Labels appear to be one-hot encoded. Converting to class indices.")
        y_sample = torch.argmax(y_sample, dim=1)
    print(f"Corrected label shape: {y_sample.shape}")
    
    model = SwinSSC(model_name=model_name, weights=Swin_T_Weights.DEFAULT, no_classes=output_classes)
    model = model.to(device)
    
    # FIXED: Use the class weights in the loss function
    ...
    for epoch in range(no_epochs):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            counter = 0
            torch.save(model.state_dict(), 'best_swin_ssc.pth')
