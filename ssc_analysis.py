...
# Load your results
df = pd.read_csv('./results/swin_t_ssc_results.csv')
...
plt.plot(df['Epoch'], df['Test_Accuracy'], 'ro-', linewidth=2.5, markersize=8, label='Test Accuracy (Target - FP)')
plt.plot(df['Epoch'], df['Train_Accuracy'], 'bo-', linewidth=2.5, markersize=8, label='Train Accuracy (Source - FO)', alpha=0.7)

plt.xlabel('Epoch', fontsize=14, fontweight='bold')
plt.ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
plt.title('SSC Performance: FO → FP Domain Adaptation\n(Swin-T Backbone, 50 Epochs)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/1. ssc_accuracy_curve.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/1. ssc_accuracy_curve.pdf', bbox_inches='tight')
plt.show()

# 2. Loss Components Analysis
plt.figure(figsize=(14, 8))
plt.plot(df['Epoch'], df['Train_Loss'], 'g-', linewidth=2.5, label='Classification Loss (Source)')
plt.plot(df['Epoch'], df['SSC_Loss'], 'b-', linewidth=2.5, label='SSC Loss (Target)')
...
plt.title('Loss Components: SSC Training Dynamics\n(FO → FP Domain Adaptation)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/2. ssc_loss_components.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/2. ssc_loss_components.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Computational Efficiency: Training Time per Epoch\n(Swin-T + SSC)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
plt.xticks(np.arange(0, 51, 5))

plt.tight_layout()
plt.savefig(f'{plot_dir}/3. ssc_timing_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/3. ssc_timing_analysis.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Time-Accuracy Tradeoff: SSC Domain Adaptation\n(FO → FP, Swin-T Backbone)', fontsize=16, fontweight='bold')
fig.tight_layout()
plt.savefig(f'{plot_dir}/4.ssc_time_accuracy_tradeoff.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/4. ssc_time_accuracy_tradeoff.pdf', bbox_inches='tight')
plt.show()
...
# Top: Accuracy
ax1.plot(df['Epoch'], df['Test_Accuracy'], 'ro-', linewidth=2.5, markersize=6, label='Test Accuracy (FP)')
ax1.plot(df['Epoch'], df['Train_Accuracy'], 'bo-', linewidth=2.5, markersize=6, label='Train Accuracy (FO)', alpha=0.7)
ax1.set_ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
ax1.set_title('A) SSC Performance: FO → FP Domain Adaptation', fontsize=14, fontweight='bold')
...
ax2.plot(df['Epoch'], df['SSC_Loss'], 'b-', linewidth=2.5, label='SSC Loss')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/5.ssc_combined_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/s5. sc_combined_analysis.pdf', bbox_inches='tight')
plt.show()

# 6. Quantitative Analysis Report
print("=== SSC TRAINING ANALYSIS REPORT ===")
...
print(f"  - SSC Loss: {df['SSC_Loss'].iloc[0]:.4f} → {df['SSC_Loss'].iloc[-1]:.4f} ({df['SSC_Loss'].iloc[-1]/df['SSC_Loss'].iloc[0]*100:.1f}%)")
print(f"\nTiming Analysis:")
...
summary = {
    ...
    'SSC_Loss_Start': df['SSC_Loss'].iloc[0],
    'SSC_Loss_End': df['SSC_Loss'].iloc[-1],
...
summary_df.to_csv(f'{plot_dir}/ssc_performance_summary.csv', index=False)
print(f"\nAnalysis summary saved to: {plot_dir}/ssc_performance_summary.csv")