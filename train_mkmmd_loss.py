...
import numpy as np
import pandas as pd
from data_loadone import get_loader
import torch
...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_mkmmd_1_results_{timestamp}.csv')
    ...
    for model_name in model_names:
        ...
        for epoch in range(no_epochs):
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                counter = 0
                torch.save(model.state_dict(), f'MKmmd_1_{model_name}.pth')
